...
class LCDM(object):
	...
	"""
	def __init__(self,Om0=0.3,h=0.7,Ob0=0.045,ns=0.96,sigma_8 = 0.8,z0 = 0.637, alpha=2, beta=1.5):
		self.Om0 = float(Om0)
		self.Ob0 = float(Ob0)
		self.ns = float(ns)
		self.h = float(h)
	...
	def D_H(self):
		...
		"""
		linspace of sight comoving distance as a function of redshift z
        ...
        def n(self,z):
                return ((self.beta/self.z0)/(gamma((1.+self.alpha)/self.beta)))*((z/self.z0)**self.alpha)*np.exp(-((z/self.z0)**self.beta))


        ########## Finding g(Xi) ############

        def intg(self,z3,z):
                return (1.-self.co_dis_z(z3)/self.co_dis_z(z))*self.n(z3)

        def g(self,z):  
                return quad(self.intg,z,5.,args=(z))[0]
	...
	def DPk_wh(self,k,z):
        ...
        ###############################################################    MY ADDING     #############################################
        ########## 2-D Weak Lensing Power Spectrum (WH) ############

        def intpl_wh(self,z,l):
                return (l*(l+1.)/(2.*np.pi))*((1./self.D_H())**3.)*(9./4.)*(self.Om0**2.)*((1.+z)**2.)*self.invhub(z)*((self.g(z))**2.)*self.Pk_wh((l/self.co_dis_z(z)),z)

        def Pl_wh2(self,l): 
                return quad(self.intpl_wh,0.001,5.,args=(l))[0]

        def Pl_wh(self,l): 
                return np.vectorize(self.Pl_wh2)(l)

        ########## 2-D Weak Lensing Power Spectrum (BBKS) ############

        def intpl_bbks(self,z,l,z0,alpha,beta):
                return (l*(l+1.)/(2.*np.pi))*((1./self.D_H())**3.)*(9./4.)*(self.Om0**2.)*((1.+z)**2.)*self.invhub(z)*((self.g(z,z0,alpha,beta))**2.)*self.Pk_bbks((l/self.co_dis_z(z)),z)

        def Pl_bbks2(self,l,z0,alpha,beta): 
                return quad(self.intpl_bbks,0.001,5.,args=(l,z0,alpha,beta,))[0] 

        def Pl_bbks(self,l,z0,alpha,beta): 
                return np.vectorize(self.Pl_bbks2)(l,z0,alpha,beta)
